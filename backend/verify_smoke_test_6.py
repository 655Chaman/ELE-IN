...
try:
    ...
    if not ws_res.data:
        print("   Skipping (No workspaces found)")
    else:
...
try:
    enr_res = supabase.table("campaign_enrollments").select("id").limit(1).execute()
    if not enr_res.data:
        print("   Skipping (No enrollments found)")
    else:
        test_enrollment_id = enr_res.data[0]["id"]
