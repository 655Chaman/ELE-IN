-- Migration: 037_suppression_errcode
-- Description: Upgrades suppression checks to use standard SQLSTATE 'U0001' errcode instead of fragile string matching.
...
BEGIN
    ...
    IF v_is_suppressed THEN
        RAISE EXCEPTION 'suppression_list_violation: Lead matches a suppression list rule and cannot be processed.' USING ERRCODE = 'U0001';
    END IF;
...
BEGIN
    ...
    LOOP
        ...
        INSERT INTO public.leads (
            ...
            company_name, email, domain, job_title, location, timezone
        ) VALUES (
            p_workspace_id,
            v_lead->>'linkedin_url',
            v_lead->>'first_name',
        ...
        ON CONFLICT ON CONSTRAINT uq_leads_workspace_linkedin DO UPDATE SET
            ...
            timezone = COALESCE(EXCLUDED.timezone, leads.timezone)
