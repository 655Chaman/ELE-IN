...
-- PARANOIA: total_campaigns counts ALL campaigns (active + paused + archived). NEVER filter by status=ACTIVE here. Veterans who pause all campaigns must not see total_campaigns drop to 0.
ALTER TABLE public.daily_workspace_stats ADD COLUMN IF NOT EXISTS total_campaigns integer DEFAULT 0;
