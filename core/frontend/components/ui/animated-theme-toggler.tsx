...
export const AnimatedThemeToggler = ({
  ...
  const shape = variant ?? "star"
  const isControlled = theme !== undefined
  const [internalIsDark, setInternalIsDark] = useState(false)
  const isDark = isControlled ? (theme === "system" ? internalIsDark : theme === "dark") : internalIsDark
  ...
  useEffect(() => {
    if (isControlled && theme !== "system") return
  ...
  return (
    <button
      ...
      ref={buttonRef}
      onClick={toggleTheme}
      className={cn("relative p-2 rounded-lg hover:bg-muted transition-colors flex items-center justify-center", className)}
      {...props}
    >
