...
import { Info, Users, Briefcase, Link as LinkIcon, Rocket } from 'lucide-react'

function StartOutreachMenu({ onSelect }: { onSelect: (id: string) => void }) {
  ...
  return (
    <div className="relative" ref={ref}>
      <button 
        ...
        className="flex items-center gap-2 bg-primary hover:bg-primary/90 text-primary-foreground px-4 py-2 rounded-xl font-bold transition-all shadow-md active:scale-95"
      >
        <Rocket size={16} /> Start outreach
      </button>
               ...
               {options.map(opt => (
                 <div key={opt.id} className="flex items-center gap-2 p-2 rounded-xl hover:bg-muted/50 transition-colors group cursor-pointer" onClick={() => { setIsOpen(false); onSelect(opt.id); }}>
                   ...
                   <div className="text-muted-foreground hover:text-foreground transition-colors relative cursor-help group/info">
                     <Info size={14} />
                     <div className="absolute right-0 bottom-full mb-2 w-48 p-2 bg-foreground text-background text-[11px] rounded-lg opacity-0 group-hover/info:opacity-100 pointer-events-none transition-opacity z-50">
                       {opt.info}
                     </div>
                   </div>
                 </div>
...
export function EleInDashboard() {
  ...
  const handleStartOutreach = (id: string) => { window.location.href = '/elein/campaigns/new?start=' + id; }
      ...
      <header className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/80 backdrop-blur-xl">
        <div className="max-w-[1600px] mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <NotificationBell />
            <StartOutreachMenu onSelect={handleStartOutreach} />
            {!isShowingOnboarding && (
              <button
                onClick={() => {
